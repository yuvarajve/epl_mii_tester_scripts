...
/*---------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------*/
void send_emergency_message(chanend c_rx_tx,
                            char error_type,
                            ...
                            REFERENCE_PARAM(char, canopen_state))
{
  ...
  index = find_index_in_od(0x1003);  //Error register
  if (index != -1)
  {
    read_data_from_od(index, 0, data_buffer, 1);
    ...
    if (!error_exists)
    {
          ...
          && (error_index_pointer < max_error_entries))
      {
        ...
        write_data_to_od(index, error_index_pointer + 1, data_buffer, 2);
...
void reset_error_register()
{
  ...
  if (index == -1)
  {
    ...
    {
      write_data_to_od(index, counter, data_buffer, 4);
      counter++;
    }
  }
}
