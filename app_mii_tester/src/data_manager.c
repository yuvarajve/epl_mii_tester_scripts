...
{
  for(unsigned int i=size; i<size+4;i++)
    printf("[%02d] : 0x%x\n",i,ptr[i]);
}
...
void data_handler(CHANEND_PARAM(chanend, c_listener_to_data_handler),CHANEND_PARAM(chanend, c_data_handler_to_tx))
{
  ...
  while(1) {
    ...
    if( (event_flag == EVENT_WAIT) || (event_flag == EVENT_EOF) ) {
      ...
      data_index = (packet_control->frame_len-(PKT_DELAY_BYTES+PKT_SIZE_BYTES));
    }
    ...
    if(event_flag == EVENT_EOP) {
      unsigned idx = 0;
      printf("\nDATA_HANDLER: no_of_packets : %d\n\n",packet_number+1);
      printf("+----------------------------------------+\n");
      printf("| ## | TSize | RSize | Tick |  Checksum  |\n");
      printf("+----------------------------------------+\n");
